...
#define ROOT_TESTDATA "D:/Projects/Ilwis/Ilwis4/testdata"
...
void LoaderTests::gdalConnector()
{
    try{
        ...
        map2->connectTo(makeUrl(map2->name()), "GTiff","gdal",Ilwis::IlwisObject::cmOUTPUT);
...
void LoaderTests::domainandtableAccess() {
    ...
    try{
        if ( loaders5) {
            ...
            res = QString("file:///%1/Kenya.tbt").arg(ROOT_TESTDATA);
            qDebug() << "Table: changing values by string";

            qDebug() << "changing to value inside the domain" ;
            tbl.prepare(res);
            tbl->setCell(0,1,"KE05");
            QVERIFY2(tbl->cell(0,1) == 3, ISSUE(tbl));

            qDebug() << "changing to value outside the domain, should fail" ;
            tbl->setCell(0,1,"Nonsense");
            QVERIFY2(tbl->cell(0,1).toInt() == iUNDEF, ISSUE(tbl));

            qDebug() << "changing to value to undefined" ;
            tbl->setCell(0,1, sUNDEF);
            QVERIFY2(tbl->cell(0,1).toInt() == iUNDEF, ISSUE(tbl));
...
void LoaderTests::scriptExecution() {
    try{
        if (loaders8) {
            ...
            qDebug() << "basic function on raster : add";
            Operation opadd({"script aa18=n000302.mpr + 10"});
            ctx.clear();
            QVERIFY2(opadd->execute(&ctx,symtbl) == true,"operation done");
            Ilwis::IRasterCoverage mapAdd = symtbl.getValue(ctx._results[0]).value<Ilwis::IRasterCoverage>();
            QVERIFY2(mapAdd->pix2value(Pixel(327,266)) == 34,ISSUE(map3));
            ...
            qDebug() << "using placeholder names";
            IRasterCoverage mpPlaceHolder;
            QString res = QString("file:///%1/small3.mpr").arg(ROOT_TESTDATA);
            mpPlaceHolder.prepare(res);
            OperationExpression opexp(QString("aa20=sin(%1%2)").arg(NAME_ALIAS).arg(mpPlaceHolder->id()));
            Operation op23(opexp);
            QVERIFY2(op23->execute(&ctx,symtbl) == true,"operation done");
            dump(ctx, symtbl, itRASTER);
...
QUrl LoaderTests::makeUrl(const QString& name) {
    QString outputname = context()->workingCatalog()->filesystemLocation().toLocalFile();
    outputname += "/" + name;
